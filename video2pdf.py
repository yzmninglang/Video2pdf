...
FGBG_HISTORY = FRAME_RATE * 9   # no.of frames in background object
VAR_THRESHOLD = 15               # Threshold on the squared Mahalanobis distance between the pixel and the model to decide whether a pixel is well described by the background model.
...
MAX_PERCENT = 2                  # max % of diff between foreground and background to detect if frame is still in motion
